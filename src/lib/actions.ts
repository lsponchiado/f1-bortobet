...
export async function requestPasswordReset(prevState: unknown, formData: FormData) {
  ...
  try {
    ...
    console.error("[reset] Erro ao enviar e-mail:", err);
...
export async function resetPassword(prevState: unknown, formData: FormData) {
  ...
  if (!email || !code || !password) return { error: "Preencha todos os campos.", step: "code" as const };
  if (password !== confirmPassword) return { error: "As senhas não coincidem.", step: "code" as const };
  if (password.length < 6) return { error: "A senha deve ter pelo menos 6 caracteres.", step: "code" as const };

  const ip = await getClientIp();
  const rl = checkRateLimit(`reset-verify:${ip}`);
  if (!rl.allowed) {
    const mins = Math.ceil((rl.retryAfterMs ?? 0) / 60000);
    return { error: `Muitas tentativas. Tente novamente em ${mins} minuto(s).`, step: "code" as const };
  }
  ...
  if (!resetEntry) return { error: "Código inválido ou expirado.", step: "code" as const };
  ...
  resetRateLimit(`reset-verify:${ip}`);

  return { success: true, step: "done" as const };
}
